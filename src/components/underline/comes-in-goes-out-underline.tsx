...
export function ComesInGoesOutUnderline<T extends ElementType = 'span'>({
  ...
  const originIn = direction === 'left' ? 'left' : 'right'
  const originOut = direction === 'left' ? 'right' : 'left'
  ...
  const animate = async () => {
    ...
    // Phase 1: line comes in from the direction side
    controls.set({ transformOrigin: originIn })
    await controls.start({ scaleX: 1, transition })

    // Phase 2: line goes out from the opposite side
    ...
    <LazyMotion features={domAnimation}>
      ...
      >
        ...
        <m.span
          ...
          initial={{ scaleX: 0, transformOrigin: originIn }}
          style={{
