...
export function GoesOutComesInUnderline<T extends ElementType = 'span'>(
  ...
  useEffect(() => {
    const updateUnderlineStyles = () => {
      ...
      const textWidth = textRef.current.querySelector('[data-underline-text]')?.getBoundingClientRect().width ?? textRef.current.getBoundingClientRect().width
  ...
  return (
    <MotionComponent
      ...
      {...props}
    >
      <span className="relative z-10 inline-block" data-underline-text>{children}</span>
      <motion.span
